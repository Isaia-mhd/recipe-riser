<form method="POST" action="{{ route('recipe.store') }}" enctype="multipart/form-data" class="bg-slate-800 max-w-lg mx-auto p-6 rounded-xl shadow-md space-y-6">
    @csrf
    ...
    <!-- Ingredients -->
    <div>
        <label for="ingredients" class="block text-sm font-medium text-gray-300 pb-2">Ingredients</label>
        <textarea name="ingredients" id="ingredients" rows="4"
                  class="w-full bg-slate-900 py-2 rounded-md border-none outline-blue-700 outline-4 px-3 text-white block" placeholder="What are the ingredients..."></textarea>
        @error('ingredients')
        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
    </div>
    ...
    <!-- Instructions -->
    <div>
        <label for="instructions" class="block text-sm font-medium text-gray-300 pb-2">Instructions</label>
        <textarea name="instructions" id="instructions" rows="5"
                  class="w-full bg-slate-900 py-2 rounded-md border-none outline-blue-700 outline-4 px-3 text-white block" placeholder="Give the instructions..." ></textarea>
        @error('instructions')
        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
    </div>
